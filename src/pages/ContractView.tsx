    ...
    const fetchJsonFromApi = async () => {
      ...
      try {
        const response = await axios.post(
          "https://51a3e11d13f2.ngrok-free.app/extract-contract-from-s3",
          {
            s3_uri: s3Uri,
  ...
  const getTableView = () => {
    return Object.entries(contractJsonData).map(([key, value]) => {
      ...
      return (
        <div
          key={key}
          className="grid grid-cols-[auto_auto_1fr] items-start gap-2 mb-2 mt-2"
        >
          ...
          <Card className="h-full rounded-none">
            ...
            <CardContent className="h-full overflow-hidden p-0">
              {loadingJson ? (
                ...
                <div className="h-[620px] overflow-y-auto border-t-2 px-2 pt-3">
